...
class Benchmark {
 ...
 private:
  ...
  void DoWrite(ThreadState* thread, bool seq) {
    ...
    for (int i = 0; i < num_; i += entries_per_batch_) {
      ...
      for (int j = 0; j < entries_per_batch_; j++) {
        ...
        char key[100];
        GenerateKey(&key[0], sizeof(key), k);
  ...
  uint64_t my_strtoul(const char *start, int size) {
    uint64_t result = 0;
  ...
  void CalculateSum(ThreadState* thread) {
    ...
    for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
      sum += my_strtoul(iter->key().data(), iter->key().size());
    }
    thread->stats.FinishedSingleOp();
    char msg[100];
    snprintf(msg, sizeof(msg), "(sum is %lu)", sum);
    thread->stats.AddMessage(msg);
    delete iter;
  }
  ...
  void CountIf(ThreadState* thread) {
    Iterator* iter = db_->NewIterator(ReadOptions());
    const char *postfix = "77";
    const int postfix_len = 2;
    uint64_t count = 0;
    for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
      int key_size = iter->key().size();
      if (key_size >= postfix_len
              && 0 == memcmp(iter->key().data() + key_size - postfix_len,
                              postfix, postfix_len))
        count++;
    }
    thread->stats.FinishedSingleOp();
    char msg[100];
    snprintf(msg, sizeof(msg), "(count is %lu)", count);
    thread->stats.AddMessage(msg);
    delete iter;
  }
  ...
  void ReadMissing(ThreadState* thread) {
    ...
    for (int i = 0; i < reads_; i++) {
      char key[100];
      const int k = thread->rand.Next() % FLAGS_num;
      GenerateMissingKey(&key[0], sizeof(key), k);
  ...
  void GenerateKey(char *buffer, size_t buffer_size, int k) {
    if (key_size_ == 16)
      ...
      snprintf(buffer, buffer_size, "%08d", k);
    else {
